...
class SettingsDialogPanel(dialog_base.SettingsDialogPanel):
    def __init__(self, parent, extra_data_func, config_save_func):
        ...
        self.notebook.AddPage(self.general, u"通用")
        self.notebook.AddPage(self.html, u"Html 默认")
        self.notebook.AddPage(self.extra, u"额外字段")
...
class GeneralSettingsPanel(dialog_base.GeneralSettingsPanelBase):
    FILE_NAME_FORMAT_HINT = (
        ...
        "\n" +
        u"    %f : 原始PCB文件名（不带扩展名）。\n" +
        u"    %p : 来自于PCB元数据中的PCB/工程标题。\n" +
        u"    %c : company from pcb metadata.\n" +
        u"    %r : revision from pcb metadata.\n" +
        u"    %d : pcb date from metadata if available, " +
        u"文件修改日期。\n" +
        u"    %D ：BOM 生成日期。\n" +
        u"    %T ：BOM 生成时间。\n" +
        u"\n" +
    ...
    def OnNameFormatHintClick(self, event):
        wx.MessageBox(self.FILE_NAME_FORMAT_HINT, u"文件名格式帮助",
